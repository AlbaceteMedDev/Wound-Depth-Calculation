...
function calculateGrid() {
  ...
  if (isNaN(full) || isNaN(partial)) { alert('Please enter the number of full and partial squares.'); return; }
  const aConform = full + (partial * 0.5);
  const fixation = perimeter > 0 ? perimeter * 0.5 : 0;
  const total = aConform + fixation;
  showResults({ method: 'Grid Overlay', aConform, perimeter, fixation, total,
    inputs: `Full squares: ${full}, Partial squares: ${partial}` + (perimeter > 0 ? `, Perimeter: ${perimeter} cm` : '') });
}
...
function calculateRectangle() {
  ...
  if (isNaN(length) || isNaN(width)) { alert('Please enter the template length and width.'); return; }
  const aConform = length * width;
  if (isNaN(perimeter) || perimeter <= 0) { perimeter = 2 * (length + width); }
  const fixation = perimeter * 0.5;
  const total = aConform + fixation;
  showResults({ method: 'Bounding Rectangle', aConform, perimeter, fixation, total,
...
function calculateElliptical() {
  ...
  if (isNaN(a) || isNaN(b)) { alert('Please enter the major and minor axes.'); return; }
  ...
  const fixation = perimeter * 0.5;
  const total = aConform + fixation;
  showResults({ method: 'Elliptical Approximation', aConform, perimeter, fixation, total,
...
function calculateTraditional() {
  ...
  if (isNaN(length) || isNaN(width) || isNaN(depth)) { alert('Please enter length, width, and depth.'); return; }
  ...
  const fixation = perimeter * 0.5;
  const total = aConform + fixation;
  showResults({ method: 'Traditional (L x W x D)', aConform, perimeter, fixation, total,
    inputs: `Length: ${length} cm, Width: ${width} cm, Depth: ${depth} cm`,
    traditional: { volume, correctedEstimate } });
}
...
function renderGraftOptions(totalArea) {
  ...
  GRAFT_SIZES.forEach(size => {
    ...
    if (!isMatch) div.classList.add('too-small');
...
function renderDocumentation(data) {
  ...
  for (const size of GRAFT_SIZES) {
    if (size.area >= data.total) { graftRec = size.label + ' (' + size.area + ' cm\u00B2)'; break; }
  }
...
function saveToHistory(data) {
  ...
  history.unshift({ timestamp: new Date().toISOString(), method: data.method, inputs: data.inputs, aConform: data.aConform, total: data.total });
...
function renderHistory() {
  ...
  history.slice(0, 10).forEach(entry => {
    ...
    div.innerHTML = `<div><strong>${entry.method}</strong> &mdash; ${entry.total.toFixed(1)} cm&sup2;<div class="history-meta">${entry.inputs}</div></div><div class="history-meta">${dateStr} ${timeStr}</div>`;
